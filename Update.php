...
<div class="container-fluid">
    <div class="row justify-content-center">
        ...
        <?php endif ?>
        <div class="col-md-4">
            <h2 id="update-title" class="text-center">Modification de <?= $user_infos['first_name'] . ' ' . $user_infos['last_name'] ?></h2>
            <form id="form-update" class="text-center" action="" method="post">
                <div class="mb-3">
                    <label class="mb-3" for="first_name">Prénom</label>
                    <input name="first_name" class="form-control" id="first_name" type="text" value="<?= $user_infos['first_name'] ?>">
                    <p class="mb-0 text-danger"><?= $errors['first_name'] ?? '' ?></p>
                </div>
                <div class="mb-3">
                    <label class="mb-3" for="last_name">Nom</label>
                    <input name="last_name" class="form-control" id="last_name" type="text" value="<?= $user_infos['last_name'] ?>">
                    <p class="mb-0 text-danger"><?= $errors['last_name'] ?? '' ?></p>
                </div>
                <div class="mb-3">
                    <label class="mb-3" for="birth_date">Date de naissance</label>
                    <input  value="<?= date('Y-m-d',strtotime($user_infos['birth_date'])) ?>" name="birth_date" class="form-control" id="birth_date" type="date">
